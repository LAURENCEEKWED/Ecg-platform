...
export default function MessagesPage() {
  ...
  const deleteMessage = async (msgId) => {
    try {
      // For demo purposes, we'll just update the UI even if API fails
      // In real app, you'd call the API here
  ...
  return (
    <AppLayout navItems={navItems} title={t('messages')}>
      <div style={{ display: 'flex', height: 'calc(100vh - 160px)', gap: '24px' }}>
        <div style={{ width: '350px', background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border)', overflow: 'hidden', display: 'flex', flexDirection: 'column', position: 'relative' }}>
          {currentConversation ? (
            <>
              <div style={{
                padding: '16px',
